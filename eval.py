...
def load_models(args):
    ...
    if vocoder_type == "bigvgan":
        ...
        vocos, _, _, _ = load_checkpoint(
            vocos, None, vocos_checkpoint_path, load_only_params=True, ignore_modules=[], is_distributed=False
        )
    ...
    if speech_tokenizer_type == "whisper":
        ...
        def semantic_fn(waves_16k):
            ori_inputs = whisper_feature_extractor(
                [waves_16k.squeeze(0).cpu().numpy()], return_tensors="pt", return_attention_mask=True
            )
    ...
    for source_i, source_line in enumerate(tqdm(source_audio_list)):
        ...
        for target_i, target_name in enumerate(target_audio_list):
            ...
            if os.path.exists(osp.join(conversion_result_dir, source_index, f"{target_name}")):
                ...
                vc_wave_16k, _ = librosa.load(osp.join(conversion_result_dir, source_index, f"{target_name}"), sr=16000)
