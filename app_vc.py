...
def get_trained_models(runs_dir="runs/v1"):
    trained_models = []
    if os.path.exists(runs_dir):
        for model_name in os.listdir(runs_dir):
            model_path = os.path.join(runs_dir, model_name)
            if os.path.isdir(model_path):
                checkpoint = os.path.join(model_path, "ft_model.pth")
                config = os.path.join(model_path, "config_dit_mel_seed_uvit_whisper_small_wavenet.yml")
                if os.path.exists(checkpoint) and os.path.exists(config):
                    audio_files = []
                    for f in os.listdir(model_path):
                        ext = os.path.splitext(f)[1].lower()
                        if ext in AUDIO_EXTENSIONS:
                            audio_files.append(os.path.join(model_path, f))
                    audio_files.sort()
                    trained_models.append((model_name, checkpoint, config, audio_files))
...
def reload_model(model_choice):
    global model, semantic_fn, vocoder_fn, campplus_model, to_mel, mel_fn_args
    global overlap_wave_len, max_context_window, sr, hop_length, args

    yield "正在加载模型，请稍候...", None

    auto_audio = None
    if model_choice == "默认模型 (HuggingFace)":
        args.checkpoint = None
        args.config = None
    else:
        trained_models = get_trained_models()
        for name, ckpt, cfg, audio_files in trained_models:
            if name == model_choice:
                args.checkpoint = ckpt
                args.config = cfg
                if audio_files:
                    auto_audio = audio_files[0]
                break
    ...
    status_msg = f"已切换到模型: {model_choice}"
    if auto_audio:
        status_msg += f"\n已自动加载参考音频: {os.path.basename(auto_audio)}"
    yield status_msg, auto_audio
...
def main(parsed_args):
    ...
    trained_models = get_trained_models()
    model_choices = ["默认模型 (HuggingFace)"] + [name for name, _, _, _ in trained_models]
    ...
    with gr.Blocks(title="Seed Voice Conversion") as demo:
        ...
        with gr.Row():
            model_dropdown = gr.Dropdown(
                ...
                value="默认模型 (HuggingFace)",
                label="选择模型 / Select Model",
                scale=4,
            )
            reload_btn = gr.Button("切换模型 / Reload Model", scale=1)
        model_status = gr.Textbox(label="状态 / Status", interactive=False, lines=2)
        ...
        reload_btn.click(
            ...
            outputs=[model_status, ref_audio_input],
            queue=True,
        )
