...
class SeedVCWrapper:
    ...
    def _load_base_model(self):
        ...
        self.model, _, _, _ = load_checkpoint(
            self.model, None, dit_checkpoint_path, load_only_params=True, ignore_modules=[], is_distributed=False
        )
    ...
    def _load_f0_model(self):
        ...
        self.model_f0, _, _, _ = load_checkpoint(
            self.model_f0, None, dit_checkpoint_path, load_only_params=True, ignore_modules=[], is_distributed=False
        )
        ...
        if audio_16k.size(-1) <= 16000 * 30:
            ...
            while traversed_time < audio_16k.size(-1):
                if buffer is None:  # first chunk
                    ...
                    chunk = torch.cat(
                        [buffer, audio_16k[:, traversed_time : traversed_time + 16000 * (30 - overlapping_time)]],
                        dim=-1,
                    )
                inputs = self.whisper_feature_extractor(
