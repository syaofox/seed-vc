...
class VoiceConversionWrapper(torch.nn.Module):
    ...
    def load_checkpoints(
        ...
        self.content_extractor_narrow.load_state_dict(content_extractor_narrow_checkpoint, strict=False)
    ...
    def compute_style(self, waves_16k: torch.Tensor, wave_lens_16k: torch.Tensor = None):
        ...
        for bib in range(waves_16k.size(0)):
            feat = torchaudio.compliance.kaldi.fbank(
                waves_16k[bib : bib + 1, : wave_lens_16k[bib]], num_mel_bins=80, dither=0, sample_frequency=16000
            )
    ...
    def convert_timbre(
        ...
        with torch.autocast(device_type=device.type, dtype=dtype):
            ...
            (
                prompt_condition,
                _,
            ) = self.cfm_length_regulator(target_content_indices, ylens=torch.LongTensor([target_mel_len]).to(device))
    ...
    def _process_content_features(self, audio_16k_tensor, is_narrow=False):
        ...
        if audio_16k_tensor.size(-1) <= 16000 * 30:
            ...
            _, content_indices, _ = content_extractor_fn(
                audio_16k_tensor, [audio_16k_tensor.size(-1)], ssl_model=self.content_extractor_wide.ssl_model
            )
        else:
            ...
            while traversed_time < audio_16k_tensor.size(-1):
                ...
                _, chunk_content_indices, _ = content_extractor_fn(
                    chunk, [chunk.size(-1)], ssl_model=self.content_extractor_wide.ssl_model
                )
                if traversed_time == 0:
