...
class AstralQuantizer(torch.nn.Module):
    ...
    def forward(self, waves_16k, wave_16k_lens, ssl_model=None):
        ...
        waves_16k_input_list = [waves_16k[bib, : wave_16k_lens[bib]].cpu().numpy() for bib in range(len(waves_16k))]
        alt_inputs = self.ssl_feature_extractor(
            waves_16k_input_list, return_tensors="pt", return_attention_mask=True, padding=True, sampling_rate=16000
        ).to(waves_16k.device)
